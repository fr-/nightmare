...
function Nightmare(options) {
  ...
  this._oldMaxListeners = process.getMaxListeners();
  ...
  this.proc.on('error', function() {
      self._detachUncaughtExceptionHandler();
      self.state = 'failed';
  });
  ...
  this._uncaughtExceptionHandlerAttached = true;
  this._attachUncaughtExceptionHandler();
...
Nightmare.prototype._attachUncaughtExceptionHandler = function() {
  var self = this;
  this._uncaughtExceptionHandler = function(err) {
    self.proc.disconnect();
    self.proc.kill();
    self._detachUncaughtExceptionHandler();
  };
  this._uncaughtExceptionHandlerAttached = true;
  process.setMaxListeners(this._oldMaxListeners + (++_instances));
  process.on('uncaughtException', this._uncaughtExceptionHandler);
};

Nightmare.prototype._detachUncaughtExceptionHandler = function() {
  if(this._uncaughtExceptionHandlerAttached) {
    this._uncaughtExceptionHandlerAttached = false;
...
Nightmare.prototype.goto = function(url, options) {
  debug('queueing action "goto" for %s', url);
  var child = this.child;
  this.queue(function(fn) {
    child.once('goto', fn);
    child.emit('goto', url, options);
  });
  return this;
};
...
Nightmare.prototype.run = function(fn) {
  ...
  var exitHandler = function(code, signal) {
      if(code !== 0) {
          done.call(self, new Error("exit child process with error code: " + code + " and signal: \"" + signal + "\""));
      }
      self.proc.removeListener('exit', exitHandler);
  };
  self.proc.on('exit', exitHandler);
  ...
  function next (err, res) {
    ...
    if (!item || this.state === 'failed') return done.apply(self, arguments);
    var args = item[1] || [];
    var method = item[0];
  ...
  function continue_fn(args, exitHandlerBind) {
    self.proc.removeListener('exit', exitHandlerBind);
    next.apply(this, args);
  }

  function exit_handler(args, recur, code) {
    if(code !== 0) {
      continue_fn.call(self, args, recur);
    }
  }
  ...
  function after (err, res) {
    ...
    var exitHandlerBind = function() {
        exit_handler.call(self, args, exitHandlerBind);
    };

    self.child.once('continue', function() {
      continue_fn.call(self, args, exitHandlerBind);
    });

    self.proc.on('exit', exitHandlerBind);
