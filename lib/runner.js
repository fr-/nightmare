...
app.on('ready', function() {
  ...
  parent.on('goto', function(url, options) {
    if (win.webContents.getUrl() == url) {
      parent.emit('goto');
    } else {
      win.webContents.loadUrl(url, options);
  ...
  parent.on('get-cookie', function(details) {
    win.webContents.session.cookies.get(details, function(err, cookies) {
      parent.emit('get-cookie', err, cookies);
    });
  });

  parent.on('set-cookie', function(details) {
    win.webContents.session.cookies.set(details, function(err) {
      parent.emit('set-cookie', err);
    });
  });

  parent.on('remove-cookie', function(details) {
    win.webContents.session.cookies.remove(details, function(err) {
      parent.emit('remove-cookie', err);
    });
  });
  ...
  parent.on('clear-cache', function() {
    win.webContents.session.clearCache(function(err) {
      parent.emit('clear-cache', err);
    });
  });
  ...
  parent.on('clear-storage', function(options) {
    win.webContents.session.clearStorageData(options, function(err) {
      parent.emit('clear-storage', err);
    });
  });
